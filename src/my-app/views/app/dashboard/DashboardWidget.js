    ...
    return (
      <Paper className="w-full rounded-8 shadow-none border-1">
        ...
        <div className="text-center pt-12 pb-28">
          <Typography
            className="text-72 leading-none text-green">{widget.data.value}</Typography>
          <Typography className="text-16" color="textSecondary">{widget.data.label}</Typography>
        </div>
        <div className="flex items-center px-16 h-52 border-t-1">
          <Typography className="text-15 flex w-full" color="textSecondary">
            <span className="truncate">{widget.data.label}</span>
            :
            <b className="pl-8">{widget.data.value}</b>
          </Typography>
        </div>
      </Paper>
    );
