...
// import { fetchDbItems } from 'my-app/containers/fetchDbItems.js';
...
class CRUDContainer extends Component {
  ...
  // whileSubscribed = ( func, arg, ) => {
  //   // if(this.state.isSubscribed) func(arg);
  //   this.state.isSubscribed && func(arg);
  // }
  ...
  Child = /* async */ ({ match: { params: { id }}}) => {
    ...
    const { getItems, } = this; // whileSubscribed,
    const {
      items,
    ...
    // getItems(path);
    // whileSubscribed( getItems, path, );
    
    isSubscribed && getItems(path);

    // // async/await ref: https://medium.freecodecamp.org/how-to-master-async-await-with-this-real-world-example-19107e7558ad
    // let items;
    // if(isSubscribed) {
    //   items = await fetchDbItems(path);
    // }
